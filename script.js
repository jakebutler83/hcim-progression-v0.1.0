...
t('m077','Choose prayer method','Safest is burying bones. Faster methods may add risk.'),calc('m078','Bury','regular bones','Prayer',10,20,4.5),calc('m079','Bury','big bones','Prayer',20,31,15),calc('m080','Bury','big bones','Prayer',31,43,15),t('m081','Confirm every player has 43 Prayer','Do not start dangerous combat milestones until all group members have Protect from Melee.'),t('m082','Stock 30 emergency teleports total','Varrock/Falador/Ardy tabs/runes/Chronicle cards, whatever you can access.'),t('m083','Stock 500+ edible food','Cakes/trout/salmon/lobsters. More is better.') ]},{phase:'Tier 3 — Dragon Scimitar route',tasks:[
...
function freshState(){return {done:{},playerDone:{},diaryDone:{},diarySteps:{},players:{groupName:'',player1:'',player2:'',player3:'',player4:'',player5:''}};}
let state=freshState();let db=null,docRef=null,progressUnsubscribe=null,remoteReady=false,saving=false;
function activePlayerCount(){return Math.max(2,Math.min(5,Number(window.HCIM_ACTIVE_GROUP?.groupSize||3)));}
function activePlayerKeys(){return Array.from({length:activePlayerCount()},(_,i)=>`player${i+1}`);}
function renderPlayerSegments(doneCount){return activePlayerKeys().map((_,i)=>`<span class="seg ${doneCount>=i+1?'on':''}"></span>`).join('');}
function localStateKey(){return `hcimProgressionState:${window.HCIM_ACTIVE_GROUP_ID||'pending'}`;}
...
async function sendDiscordUpdate(payload){
  const webhookUrl=window.ProgressPathDiscord?.getWebhook?.()||localStorage.getItem('progressPathDiscordWebhook')||'';
  if(!webhookUrl){
    return {ok:false,status:400,text:'No Discord webhook is saved in this browser.'};
  }
  const body={
    webhookUrl,
    groupName:(state.players&&state.players.groupName)||window.HCIM_ACTIVE_GROUP?.name||'HCIM Group',
    ...payload
  };
  try{
    const token=window.HCIM_CURRENT_USER?await window.HCIM_CURRENT_USER.getIdToken():'';
    body.groupId=window.HCIM_ACTIVE_GROUP_ID||'';
    ...
    if(!res.ok){
      ...
      return {ok:false,status:res.status,text:text||`Request failed (${res.status}).`};
    }
...
async function testDiscord(){
  ...
  const button=document.getElementById('discordTestBtn');
  if(button)button.disabled=true;
  if(status)status.textContent='Sending test...';
  const result=await sendDiscordUpdate({
    type:'test',
    title:'Discord test from HCIM Progression',
    player:'System',
    progress:'If you see this, Discord is connected.',
    phase:'Setup Test'
  });
  if(status)status.textContent=result&&result.ok?'Sent ✅':`Failed ❌ — ${result?.text||'check the saved webhook'}`;
  if(button)button.disabled=false;
}
function initFirebase(){const c=window.HCIM_FIREBASE_CONFIG||{};const groupId=window.HCIM_ACTIVE_GROUP_ID;if(progressUnsubscribe){progressUnsubscribe();progressUnsubscribe=null;}remoteReady=false;docRef=null;if(!c.apiKey||String(c.apiKey).includes('PASTE')||!window.HCIM_CURRENT_USER||!groupId)return false;try{if(!firebase.apps.length)firebase.initializeApp(c);db=firebase.firestore();docRef=db.collection('groups').doc(groupId).collection('progress').doc('main');progressUnsubscribe=docRef.onSnapshot(s=>{if(groupId!==window.HCIM_ACTIVE_GROUP_ID)return;if(s.exists){state={...state,...s.data(),done:{...(s.data().done||{})},playerDone:{...(s.data().playerDone||{})},diaryDone:{...(s.data().diaryDone||{})},diarySteps:{...(s.data().diarySteps||{})},players:{...(state.players||{}),...(s.data().players||{})}};normalizeState();renderAll();}else saveState();},e=>{console.warn('Progress listener stopped',e);remoteReady=false;updateSync();});remoteReady=true;return true;}catch(e){console.warn(e);return false;}}
function loadLocal(){try{const raw=localStorage.getItem(localStateKey());if(raw)state={...state,...JSON.parse(raw)};normalizeState();}catch(e){}}
function saveState(){if(saving)return;saving=true;localStorage.setItem(localStateKey(),JSON.stringify(state));if(remoteReady&&docRef)docRef.set(state,{merge:true}).catch(console.warn).finally(()=>saving=false);else saving=false;updateSync();}
function updateSync(){const el=document.getElementById('syncStatus'),details=document.getElementById('syncDetails');if(remoteReady){if(el){el.textContent='Cloud sync active';el.className='sync-status live';}if(details)details.textContent='Your group progress is connected and saves automatically.';}else{if(el){el.textContent='Offline save mode';el.className='sync-status local';}if(details)details.textContent='Cloud sync is temporarily unavailable. Changes are being kept in this browser until the connection returns.';}}
...
function playerLabel(key){const p=state.players||{};const n=Number(String(key).replace('player',''))||1;return p[key]||`P${n}`;}
function isPersonalTask(task){return personalTaskIds.has(task.id);}
function normalizeState(){state.done=state.done||{};state.playerDone=state.playerDone||{};state.diaryDone=state.diaryDone||{};state.diarySteps=state.diarySteps||{};state.players=state.players||{groupName:'',player1:'',player2:'',player3:'',player4:'',player5:''};activePlayerKeys().forEach(k=>{if(!(k in state.players))state.players[k]='';});allTasks().forEach(task=>{if(isPersonalTask(task)&&state.done[task.id]&&!state.playerDone[task.id]){state.playerDone[task.id]={};activePlayerKeys().forEach(k=>state.playerDone[task.id][k]=true);}});}
function personalCount(id){const d=(state.playerDone||{})[id]||{};return activePlayerKeys().filter(k=>!!d[k]).length;}
function taskDone(task){if(isPersonalTask(task))return personalCount(task.id)===activePlayerCount();return !!state.done[task.id];}
function setPersonalDone(id,key,value){state.playerDone=state.playerDone||{};state.playerDone[id]=state.playerDone[id]||{};state.playerDone[id][key]=value;state.done[id]=personalCount(id)===activePlayerCount();}
function setTaskDone(task,value){if(isPersonalTask(task)){activePlayerKeys().forEach(k=>setPersonalDone(task.id,k,value));}else{state.done[task.id]=value;}}
...
function renderPlayers(){
  ...
  document.querySelectorAll('.player-slot').forEach(label=>{
    const slot=Number(label.dataset.playerSlot||0);
    label.hidden=slot>count;
  });
  ['groupName','player1','player2','player3','player4','player5'].forEach(id=>{
    const el=document.getElementById(id);
    if(el&&document.activeElement!==el)el.value=(state.players||{})[id]||'';
  });
}
...
function questStatus(q){
  ...
  if(!qTask && personalCount(q.id)===activePlayerCount())return 'complete';
...
function renderQuestTracker(){
  ...
  questData.forEach(q=>{
    ...
    const pbar=`<div class="quest-player-progress"><div class="segments">${renderPlayerSegments(pc)}</div><span>${pc}/${activePlayerCount()} players complete</span></div>`;
    ...
    activePlayerKeys().forEach(k=>{const btn=document.createElement('button');btn.type='button';btn.className='player-pill '+(((state.playerDone||{})[q.id]||{})[k]?'checked':'');btn.textContent=(((state.playerDone||{})[q.id]||{})[k]?'✓ ':'')+playerLabel(k);btn.addEventListener('click',()=>{const current=!!(((state.playerDone||{})[q.id]||{})[k]);setPersonalDone(q.id,k,!current);if(!current){const count=personalCount(q.id);notifyTaskComplete({id:q.id,title:q.name,desc:q.reward,role:'All'},{type:count===activePlayerCount()?'group-complete':'player-complete',player:playerLabel(k),progress:`${count}/${activePlayerCount()} players done`,title:count===activePlayerCount()?`Group completed ${q.name}`:`${playerLabel(k)} completed ${q.name}`});}saveState();renderAll();});toggles.appendChild(btn);});
...
function diaryCount(id){const d=diaryData.find(x=>x.id===id);if(!d)return 0;return activePlayerKeys().filter(k=>diaryPlayerComplete(d,k)).length;}
function diaryComplete(d){return !!d && activePlayerKeys().every(k=>diaryPlayerComplete(d,k));}
...
function diaryStepProgress(d){const total=(d.steps||[]).length*activePlayerCount();if(!total)return 0;let done=0;(d.steps||[]).forEach((_,idx)=>activePlayerKeys().forEach(k=>{if(getDiaryStepDone(d,idx,k))done++;}));return Math.round(done/total*100);}
function tierSort(t){return {Easy:1,Medium:2,Hard:3,Elite:4}[t]||9;}
const diaryRewardItems={
  'Ardougne':'Ardougne cloak','Desert':'Desert amulet','Falador':'Falador shield',
  'Fremennik':'Fremennik sea boots','Kandarin':'Kandarin headgear','Karamja':'Karamja gloves',
  'Kourend & Kebos':"Rada's blessing",'Lumbridge & Draynor':"Explorer's ring",
  'Morytania':'Morytania legs','Varrock':'Varrock armour',
  'Western Provinces':'Western banner','Wilderness':'Wilderness sword'
};
const diaryTierNumbers={Easy:1,Medium:2,Hard:3,Elite:4};
const diaryLampXp={Easy:2500,Medium:7500,Hard:15000,Elite:50000};
function diaryRewards(d){
  const item=(diaryRewardItems[d.area]||`${d.area} diary item`)+' '+(diaryTierNumbers[d.tier]||'');
  const lamp=(diaryLampXp[d.tier]||0).toLocaleString();
  return {
    item,
    lamp,
    perks:`All ${d.tier.toLowerCase()}-tier regional benefits, including cumulative teleport, skilling, and utility upgrades.`
  };
}
function populateDiaryRegionFilter(){const select=document.getElementById('diaryRegionFilter');if(!select||select.options.length>1)return;[...new Set(diaryData.map(d=>d.area))].sort().forEach(area=>select.add(new Option(area,area)));}
function updateDiarySummary(){const slots=activePlayerKeys();const taskCount=diaryData.reduce((sum,d)=>sum+(d.steps||[]).length,0);let done=0,total=0;diaryData.forEach(d=>(d.steps||[]).forEach((_,idx)=>slots.forEach(slot=>{total++;if(getDiaryStepDone(d,idx,slot))done++;})));const values={diaryRegionCount:new Set(diaryData.map(d=>d.area)).size,diaryTierCount:diaryData.length,diaryTaskCount:taskCount,diaryOverallPct:(total?Math.round(done/total*100):0)+'%'};Object.entries(values).forEach(([id,value])=>{const el=document.getElementById(id);if(el)el.textContent=value;});}
function renderDiaryTracker(){const box=document.getElementById('diaryTracker');if(!box)return;populateDiaryRegionFilter();updateDiarySummary();box.innerHTML='';const filter=document.getElementById('diaryFilter')?.value||'all';const regionFilter=document.getElementById('diaryRegionFilter')?.value||'all';const search=String(document.getElementById('diarySearch')?.value||'').trim().toLowerCase();let shown=0;const grouped={};diaryData.slice().filter(d=>regionFilter==='all'||d.area===regionFilter).filter(d=>!search||[d.area,d.tier,d.wiki,...(d.steps||[])].join(' ').toLowerCase().includes(search)).sort((a,b)=>a.area.localeCompare(b.area)||tierSort(a.tier)-tierSort(b.tier)).forEach(d=>{(grouped[d.area]=grouped[d.area]||[]).push(d);});Object.keys(grouped).forEach(area=>{const areaWrap=document.createElement('section');areaWrap.className='diary-area-panel';const areaTiers=grouped[area];const areaComplete=areaTiers.filter(d=>diaryComplete(d)).length;const areaTaskTotal=areaTiers.reduce((sum,d)=>sum+(d.steps||[]).length*activePlayerCount(),0);const areaTaskDone=areaTiers.reduce((sum,d)=>sum+(d.steps||[]).reduce((stepSum,_,idx)=>stepSum+activePlayerKeys().filter(k=>getDiaryStepDone(d,idx,k)).length,0),0);const areaPct=areaTaskTotal?Math.round(areaTaskDone/areaTaskTotal*100):0;const regionSlug=String(area).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'');areaWrap.dataset.region=regionSlug;areaWrap.style.setProperty('--region-progress',`${areaPct}%`);areaWrap.innerHTML=`<div class="diary-region-hero"><div class="diary-region-art" aria-hidden="true"><span class="region-sun"></span><span class="region-mountains"></span><span class="region-landmark"></span><span class="region-ground"></span></div><div class="diary-region-overlay"></div><div class="diary-region-copy"><div><span class="diary-region-kicker">REGIONAL DIARY</span><h3>${diaryIcon(areaTiers[0])} ${area}</h3><p>${areaComplete}/${areaTiers.length} tiers complete • ${areaPct}% of member tasks</p></div><div class="diary-region-ring" style="--pct:${areaPct}"><strong>${areaPct}%</strong></div></div><div class="diary-region-progress"><span style="width:${areaPct}%"></span></div></div><div class="diary-tier-list"></div>`;const list=areaWrap.querySelector('.diary-tier-list');
...
list.style.setProperty('padding','16px','important');
...
card.className=`diary-card diary-dropdown ${complete?'complete':ready?'ready':'blocked'} ${diaryDifficultyClass(d.tier)}`;
card.style.setProperty('width','100%','important');
card.style.setProperty('min-width','0','important');
card.style.setProperty('max-width','none','important');card.innerHTML=`<details ${openDiaryCards.has(d.id)?'open':''}><summary><div class="diary-summary-main"><span class="diary-icon">${diaryIcon(d)}</span><div><h3>${d.tier} Diary</h3><p>${count}/${activePlayerCount()} players complete • ${stepPct}% step progress</p></div></div><span class="quest-status ${complete?'complete':ready?'ready':'blocked'}">${complete?'COMPLETE':ready?'READY':'BLOCKED'}</span></summary><div class="diary-body"><div class="difficulty-strip"><span>${d.tier}</span><span>${(d.steps||[]).length} official tasks</span></div><div class="diary-reward-box"><strong>Tier rewards</strong><div><span>🎽 ${diaryRewards(d).item}</span><span>💡 ${diaryRewards(d).lamp} XP lamp</span></div><p>${diaryRewards(d).perks}</p></div><div class="quest-player-progress"><div class="segments">${renderPlayerSegments(count)}</div><span>${count}/${activePlayerCount()} players complete full tier</span></div><div class="player-toggles diary-toggles"></div><strong>Route checks</strong><ul>${reqHtml}</ul><div class="diary-step-list"></div><a class="wiki-link big" href="${wikiUrl(d.wiki)}" target="_blank" rel="noopener">Open OSRS Wiki</a></div></details>`;const toggles=card.querySelector('.player-toggles');activePlayerKeys().forEach(k=>{const done=diaryPlayerComplete(d,k);const btn=document.createElement('button');btn.type='button';btn.className='player-pill '+(done?'checked':'');btn.textContent=(done?'✓ ':'')+playerLabel(k)+' full tier';btn.addEventListener('click',()=>{const nextVal=!diaryPlayerComplete(d,k);setDiaryDone(d.id,k,nextVal);if(nextVal){const countNow=diaryCount(d.id);sendDiscordUpdate({type:countNow===activePlayerCount()?'group-complete':'diary',title:countNow===activePlayerCount()?`Group completed ${d.area} ${d.tier} Diary`:`${playerLabel(k)} completed ${d.area} ${d.tier} Diary`,player:playerLabel(k),progress:`${countNow}/${activePlayerCount()} players done`,phase:'Achievement Diary'});}saveState();renderAll();});toggles.appendChild(btn);});const stepBox=card.querySelector('.diary-step-list');(d.steps||[]).forEach((step,idx)=>{const row=document.createElement('div');row.className='diary-step-row';const donePlayers=activePlayerKeys().filter(k=>getDiaryStepDone(d,idx,k)).length;row.innerHTML=`<div class="diary-step-title"><span class="mini-step-count">${donePlayers}/${activePlayerCount()}</span><span>${idx+1}. ${step}</span></div><div class="mini-player-toggles"></div>`;const mini=row.querySelector('.mini-player-toggles');activePlayerKeys().forEach(k=>{const b=document.createElement('button');b.type='button';b.className='mini-player '+(getDiaryStepDone(d,idx,k)?'checked':'');b.title=playerLabel(k);b.textContent=(getDiaryStepDone(d,idx,k)?'✓':playerLabel(k).slice(0,1).toUpperCase());b.addEventListener('click',()=>{const nv=!getDiaryStepDone(d,idx,k);setDiaryStepDone(d,idx,k,nv);if(nv && diaryPlayerComplete(d,k)){state.diaryDone=state.diaryDone||{};state.diaryDone[d.id]=state.diaryDone[d.id]||{};state.diaryDone[d.id][k]=true;}else if(!nv){state.diaryDone=state.diaryDone||{};state.diaryDone[d.id]=state.diaryDone[d.id]||{};state.diaryDone[d.id][k]=false;}saveState();renderAll();});mini.appendChild(b);});stepBox.appendChild(row);});const detailsEl=card.querySelector('details');
if(detailsEl){
  card.style.setProperty('grid-column',detailsEl.open?'1 / -1':'auto','important');
  detailsEl.addEventListener('toggle',()=>{
    if(detailsEl.open) openDiaryCards.add(d.id);
    else openDiaryCards.delete(d.id);
    card.style.setProperty('grid-column',detailsEl.open?'1 / -1':'auto','important');
  });
}
list.appendChild(card);});if(areaShown)box.appendChild(areaWrap);});if(!shown)box.innerHTML='<div class="empty-state"><strong>No diaries found.</strong><span>Try clearing the search or changing the region/progress filters.</span></div>';}
function enforceDiaryResponsiveGrid(){
  document.querySelectorAll('.diary-tier-list').forEach(list=>{
    list.style.setProperty(
      'grid-template-columns',
      window.innerWidth<=900?'1fr':'repeat(2, minmax(0, 1fr))',
      'important'
    );
  });
}
window.addEventListener('resize',enforceDiaryResponsiveGrid);
...
function renderAll(){safeRenderBlock('normalize',normalizeState);safeRenderBlock('phase filter',populateFilter);safeRenderBlock('players',renderPlayers);safeRenderBlock('main route',()=>renderRoute('mainTasks',route));safeRenderBlock('player jobs',()=>renderRoute('teamTasks',teamRoute));safeRenderBlock('group storage',()=>renderRoute('storageTasks',storageRoute));safeRenderBlock('quest tracker',renderQuestTracker);safeRenderBlock('diary tracker',renderDiaryTracker);safeRenderBlock('diary responsive grid',enforceDiaryResponsiveGrid);safeRenderBlock('dashboard',renderDashboard);safeRenderBlock('skill math',renderCalc);safeRenderBlock('sync status',updateSync);}
